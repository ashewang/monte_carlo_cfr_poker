...
def learn(iterations, cards, num_cards, node_map, action_map):
    if len(cards) > 4:
        from state import Leduc as State
        from hand_eval import leduc_eval as eval
    else:
        from state import State
        from hand_eval import kuhn_eval as eval
            ...
            for action in valid_actions:
                if prune is True and leaf is False and node.regret_sum[action] <= REGRET_MIN:
                    ...
                    if leaf is True:
                        ...
                        new_state = state.take(action, deep=True)
                        returned = self.accumulate_regrets_search(traverser, new_state, node_map, action_map,
                                                                  continuations,
